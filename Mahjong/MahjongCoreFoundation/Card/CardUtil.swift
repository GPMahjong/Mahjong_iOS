...
extension Card {
    // IsSuit 是否普通牌
    // 普通牌是指万、筒、条
    static func IsSuit(card int) -> Bool {
    ...
    // 九条、九筒、九万只有自己和下一张
    // 非万、筒、条 只有自己
    static func GetSelfAndNeighborCards(cards ...int) []int {
        result := []int{}
    ...
    // 包括自己、相邻的、跳张
    static func GetRelationTiles(cards ...int) []int {
        result := []int{}
        for _, card := range cards {
            result = append(result, card)
            // 非普通牌、只有自身
    ...
    static func GetFrontCardCycle(card int) int {
        var front int
        if IsSuit(card) {
            if util.IntInSlice(card, LeftSideCards) {
                front = card + 8
